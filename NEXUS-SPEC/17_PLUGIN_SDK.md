...
- **Backend plugin** — contributes one or more _integrations_: manifest-declared execution +
...
- **UI plugin** — contributes panels, inspector sections, commands, context-menu items, node type
  _renderers_, and settings. Executes in a sandboxed iframe in the browser.
...
import {
  zSemver,
  zEntityKind,
  zPermission as zIntegrationPermission,
  zInputField,
  zOutputSpec,
  zExecution,
  zEntityMapping,
...
export const zContributionCommand = z.object({
  ...
  keybinding: z
    .string()
    .regex(/^(Ctrl|Alt|Shift|Meta)(\+(Ctrl|Alt|Shift|Meta))*\+[A-Za-z0-9]$/)
...
export const zContributionNodeType = z.object({
  ...
  card: z.object({
    ...
    accent: z
      .string()
      .regex(/^--[a-z0-9-]+$/)
      .default('--node-accent-default'), // design token only
...
export const zPluginManifest = z
  .object({
    ...
    bundle: z.object({
      ...
      csp: z
        .object({
          // extra CSP the plugin needs, allowlisted
  ...
  .superRefine((m, ctx) => {
    ...
    for (const item of m.contributes.contextMenuItems)
      if (!cmdIds.has(item.commandId))
        ctx.addIssue({
          code: 'custom',
          path: ['contributes', 'contextMenuItems'],
...
by default. Those objects then flow through the _unchanged_ pipeline of `10_INTEGRATIONS.md` §3.
...
stays unbound, and Settings → Keyboard shows "Requested Ctrl+Shift+W — taken by _Close board_"
...
export default definePlugin({
  ...
  async deactivate() {
    /* release resources; ≤ 500 ms budget */
  },
});
```
...
export interface GraphApi {
  ...
  getNeighbors(
    id: string,
    depth?: 1 | 2,
  ): Promise<{ nodes: readonly ReadonlyNode[]; edges: readonly ReadonlyEdge[] }>;
...
export interface UiApi {
  ...
  progress<T>(
    title: string,
    fn: (report: (p: { message?: string; percent?: number }) => void) => Promise<T>,
  ): Promise<T>;
...
`ui.notify` and `ui.confirm` always render with a "from _<plugin name>_" attribution line; a plugin
...
- Requests and denials are logged per plugin and visible in Settings → Plugins → _Network activity_
...
export interface EventsApi {
  // requires events:subscribe + manifest.events
...
export const zEnvelope = z.discriminatedUnion('t', [
  ...
  z.object({
    ...
    method: z.string(),
    args: z.unknown(),
    deadlineMs: z.number().int().max(30_000),
  }),
  z.object({
    t: z.literal('result'),
    id: z.string().uuid(),
...
The "will NOT" block is generated from the _unrequested_ permissions and is mandatory — users judge
...
Rollback: the previous artifact is retained for 90 days; Settings → Plugins → _Version history_
...
const host = createTestHost({
  ...
  net: {
    // deterministic fetch mocking; unmocked hosts throw
    'https://archive.org/wayback/available?url=example.com%2Fa': {
      ...
      body: {
        archived_snapshots: {
          closest: {
            available: true,
            url: 'http://web.archive.org/web/20200101/https://example.com/a',
...
The testkit implements the _same_ RPC router and validators as the host (imported, not
...
```json
{
  ...
  "contributes": {
    ...
    "inspectors": [
      {
        "id": "wb-inspector",
        "title": "Wayback",
        "entry": "ui/inspector.js",
        "appliesTo": { "nodeTypes": [], "entityKinds": ["url"] },
        "order": 600,
        "maxHeightPx": 160
      }
    ],
    "settings": [
      {
        "key": "preferredYear",
        "label": "Preferred snapshot year",
        "type": "string",
        "help": "Empty = closest available.",
        "scope": "project"
      }
    ]
  },
  ...
  "network": {
    "allow": ["archive.org", "web.archive.org"],
...
export default definePlugin({
  async activate(ctx: PluginContext) {
    ctx.commands.register('snapshot', async ({ selection }) => {
      ...
      const found = await ctx.ui.progress('Searching the Wayback Machine', async (report) => {
        const acc: {
          node: ReadonlyNode;
          snap: NonNullable<NonNullable<AvailabilityResponse['archived_snapshots']>['closest']>;
        }[] = [];
      ...
      if (found.length === 0) {
        await ctx.ui.notify({
          level: 'info',
          message: 'No snapshots found for the selected URLs.',
        });
        return;
      }
  ...
  async deactivate() {
    /* nothing to release: no timers, no listeners beyond ctx-owned ones */
  },
});
...
function Inspector() {
  ...
  useEffect(
    () =>
      connectInspector(async (node, ctx) => {
  ...
  if (!state.snapshot)
    return (
      <p class="empty">
        No snapshot looked up yet. Use <b>Find Wayback snapshot</b>.
      </p>
    );
  return (
...
export interface RegistryEntry {
  ...
  manifestPreview: Pick<
    PluginManifest,
    'name' | 'description' | 'permissions' | 'contributes' | 'maturity'
  >;
