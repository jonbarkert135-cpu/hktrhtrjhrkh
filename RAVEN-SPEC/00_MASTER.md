...
| AI layer          | Provider-abstracted (`AIProvider` interface), **keyless by policy**: one OpenAI-compatible transport with a user-configured base URL (local Ollama/llama.cpp/LM Studio by default, g4f or any other endpoint opt-in, off by default, never enabled server-side); **all writes go through a Proposal object** | model choice must be swappable; AI never mutates the graph directly                                                                                                                                 |
...
| P13   | AI layer (local/keyless)      | summarize, explain, suggest links, dedupe, cluster, investigation summary      |
