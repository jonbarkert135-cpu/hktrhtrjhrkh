...
const { rule } = require('../eslint/rules/no-hardcoded-design-values.cjs') as { rule: Rule.RuleModule };
  ...
  it('accepts tokens and rejects raw design values', () => {
    tester.run('no-hardcoded-design-values', rule, {
      valid: [
        ...
        { code: 'const n = 240;' },
        { code: "const hex = '#0f172a';", filename: 'packages/ui/src/tokens/colors.ts', options: [{ allowFiles: ['packages/ui/src/tokens/'] }] },
      ],
      invalid: [
